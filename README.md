...
终端扫描结果、Web 扫描详情与新生成的 Markdown 报告统一显示关注项分类数和原始观察数，
例如 `2 类（104 条原始观察）`；空结果为 `0 类（0 条原始观察）`。
分组沿用标题、类别、严重性、置信度、说明和修复建议六字段，原始发现与证据不会被删除。
API 的 `finding_count` 仍是原始记录数，新增可选字段 `finding_group_count` 表示分类数。
兼容旧响应时若该字段缺失，显示 `104 条原始观察（分类数未提供）`，不将未知分类数当成 0。
历史报告文件不会自动改写；coverage 的 `shared/user_only/admin_only/unknown` 仍是独立覆盖分类。
